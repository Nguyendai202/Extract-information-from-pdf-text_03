...
def parser(text, label):
    if label == 'LOAIGIAY':
        text = text.replace('T H ải', "Cải")
    if label == 'CHUHOSO':
        text = re.sub(r'\bê\b', 'Lê', text)
        text = re.sub(r'\bÊ\b', 'LÊ', text)
    elif label == 'NGUOIKY':
        text = re.sub(r'\d+', '', text)
    elif label == 'NGAYSINH':
        text = re.sub(r'[^0-9/]', '', text)
    elif label == 'SOGIAYTOCAICHINH':
        text = re.sub(r'\D', '', text)
    elif label == 'SOGIAYTOTUYTHAN':
        text = re.sub(r'\D', '', text)
    elif label in ('NGAYCAPGIAYTOTUYTHAN', 'NGAYCAPGIAITOCAICHINH'):
        text = re.sub(r'[^0-9/]', '', text)
    elif label == 'NOIDUNGTRUOCCAICHINH':
        text = re.sub(r'\bê\b', 'Lê', text, 1)
        text = re.sub(r'\bÊ\b', 'LÊ', text, 1)
        text = text.replace('Dƣơng', 'Dương')
    elif label == 'NOIDUNGSAUCAICHINH':
        text = re.sub(r'\bê\b', 'Lê', text, 1)
        text = re.sub(r'\bÊ\b', 'LÊ', text, 1)
        text = text.replace('Dƣơng', 'Dương')
    elif label == 'TRICHYEU':
        ...
        trich_yeu = text_trichyeu
        numbers_in_trichyeu = re.findall(r'\d+', trich_yeu)
        for number in numbers_in_trichyeu:
...
def getPredictions(pdf_path):
    ...
    # entitis = dict(TRICHYEU=[], SOGIAYTOCAICHINH=[], NGAYBANHANH=[], LOAIGIAY=[], CHUHOSO=[
    # ], NGAYSINH=[], GIOITINH=[], DANTOC=[], QUOCTICH=[],LOAIGIAYTOTUYTHAN=[],SOGIAYTOTUYTHAN=[],NOICAPGIAYTOTUYTHAN=[],
    # NGAYCAPGIAYTOTUYTHAN=[],NOICUTRU=[],NOIDUNGTRUOCCAICHINH=[],NGAYCAPGIAITOCAICHINH=[],NOIDUNGCAICHINH=[],NOIDUNGSAUCAICHINH=[],
    # CHUCVU=[],NGUOIKY=[])
